...
const FactCheck: React.FC<FactCheckProps> = ({ factCheckData, isLoading, error }) => {
  if (isLoading) {
    return (
      <CardContent>
        <CircularProgress />
  ...
  if (error) {
    return (
      <CardContent>
        <Typography color="error">Error loading fact check data: {error.message}</Typography>
      </CardContent>
    );
  }
      ...
      {Object.entries(videoData.fact_checks).map(([queryString, queryData]) => (
        <Box key={queryString} style={{marginBottom: '20px'}}>
          ...
          {queryData.claims.map((claim, claimIndex) => (
            <Box key={`${queryString}-${claimIndex}`} style={{marginBottom: '15px', marginLeft: '15px'}}>
              <Typography variant="body2" color="text.secondary">
                {claim.text}
              </Typography>
              <Typography variant="caption" color="text.secondary">
                Claimant: {claim.claimant} | Date: {new Date(claim.claimDate).toLocaleDateString()} | Link: {claim.claimReview[0].url}
              </Typography>
              <br />
